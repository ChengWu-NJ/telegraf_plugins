...
func (a *Agent) startServiceInputs(
    ...
    startedwc := []telegraf.ServiceInputWithContext{}

    for _, input := range a.Config.Inputs {
        si, siok := input.Input.(telegraf.ServiceInput)
        siwc, siwcok := input.Input.(telegraf.ServiceInputWithContext)
        ...
        if siok || siwcok {
            ...
            if siwcok {
                startedwc = append(startedwc, siwc)
            } else {
...
func (a *Agent) stopServiceInputs() {
    for _, input := range a.Config.Inputs {
        if si, siok := input.Input.(telegraf.ServiceInput); siok {
            si.Stop()
        } else if siwc, siwcok := input.Input.(telegraf.ServiceInputWithContext); siwcok {
            siwc.Stop()
        }
    }
}
